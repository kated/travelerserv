		...
		if (count($dataset) > 0) {
			for ($i=0;$i<16;$i++) {
				...
				if ($dataset[$j+0] != "" && $dataset[$j+1] != "") {
					$query = "INSERT INTO log VALUES ('','".$_SESSION['username']."','".$day."','".$dataset[$j+0]."','".$dataset[$j+1]."','".$dataset[$j+2]."','".$dataset[$j+3]."','".$dataset[$j+4]."','".$dataset[$j+5]."','".$dataset[$j+6]."','".$dataset[$j+7]."','".$dataset[$j+8]."')";
