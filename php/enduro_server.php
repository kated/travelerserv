<?php 
	...
	socket_bind($socket, '128.111.73.56', 3002);
	...
	$previousActLat = 0;
	$previousActLon = 0;
	do {	// endless loop, constantly check for incoming packets
		socket_recvfrom($socket, $buf, 4096, 0, $from, $port);		// Receive data from the device
		// echo $buf . "\n";
		
		$aEnduro = explode(",",$buf);	// split based on comma
		if ($aEnduro[0] == '+RESP:GTRTL' or $aEnduro[0] == '+RESP:GTTRI') {  // if it is REAL TIME LOCATE or FIXED TIME REPORT 	
			if ($aEnduro[10] != 0 and $aEnduro[11] != 0) {	// and it has a valid lat/lon
				...
				if ($previousdate != $aEnduro[12]) {	// catch duplicate time stamps
					...
					DBAddFix($aEnduro, 'EnduroGPS');		// send it to the database
